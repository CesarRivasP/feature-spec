...
6. Scaffold `_log.md` from `templates/_log.md.tpl` — **at step 2, before filling the registry, not here** — and complete its `R1 · author` entry now: what you wrote, and what you left `basis: asserted`. Skip only for a set you know will never leave this session; adding it later costs the history you already lost.
...
- **The log outranks the context window.** A spec set outlives the session that wrote it and is often worked by several models. An agent that reviews its own context instead of the file on disk validates against a version that may be two rounds old, and nothing in its output says so. Read `_log.md` first, re-read at the versions it names, and open an entry before changing anything — completing it at the end — recording the line count and hash of what you read is what makes "I reviewed doc 02" falsifiable instead of merely stated. `references/handoff.md`.
...
- **Shipping with a cause still asserted is allowed, once, in writing, with an expiry date.** `defects[].accepted: { by, decided_on, until, because }` — a person, not an agent. Deciding to ship the trimmed scope and measure later is a real call; what makes it honest is that it is *recorded where the audit reads it*, and that it expires. Writing it in `_log.md` or leaving it in an agent's memory does not count: the log is narrative and a memory is private, and neither can be checked against a date. *Real case:* exactly that happened, and the same G1 violation was re-derived days later by a different session that had no way to know it was already settled.
- **Wrong entries are retracted or corrected, never deleted and never silently edited.** `retracted_on:` + `retracted_by:` when it no longer holds, `corrected_on:` + `correction:` when it holds amended; a criterion nobody can satisfy any more is `status: retired` with `retired_on:` + `retired_because:`, not a deletion. A deleted entry gets re-derived and breaks every citation of it; a retracted one keeps both, and the audit tells every citation that still leans on it — in this set and in the sets that cite it (checks 26, 36). *Real case:* retiring three criteria to ship a research set meant deleting them and rewriting their mentions by hand.
- **A conclusion read off other measurements is derived, not re-measured.** `evidence: { derived_from: [limits.x, limits.y], date, value }` inherits its sources' `n:`/`spread:` and is `measured` only while all of them are (check 35). Copying the bar is two copies of one datum.
- **A measurement is a run, not a number.** `value:` says what came back; it says nothing about how many times you looked, what varied while you looked, or whether the run happened at all. `n:`, `spread:`, `conditions:` on every `how: device|log|sentry` measurement; `outcome: aborted_no_conditions` when a precondition could not be met; `absence:` + `emits:` + `sample_rate:` when the value is the absence of a signal. Four retractions, one shape — a run reported as a constant.
