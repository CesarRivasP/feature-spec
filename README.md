...
Restart the session so the skill is picked up. No dependencies — 18 markdown and YAML files, no scripts, no MCP servers, no package to install.
...
`verify` is the only one of the four that can fail after a clean `audit`. `status: shipped` is gated on it: the set may not claim to have shipped while the defect it blames is still `basis: asserted`.
...
references/
  ...
  audit-protocol.md                   16 mechanical checks + severity taxonomy
...
templates/                            _facts.yml.tpl + one .tpl per doc
