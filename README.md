...
```bash
git clone --branch v1.11.1 --depth 1 git@github.com:CesarRivasP/feature-spec.git
```
...
|---|---|
| [**v1.11.1**](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.11.1) | Check 26 failed open on an `acceptance[].status` outside its vocabulary: the value matched no rule and passed `shipped` in silence. Two real sets carried `status: verified` — four criteria in a shipped research set, one in its parent — reading like `approved` and meaning nothing to the audit. Any value other than `written | executed | approved | retired` is now `DRIFT`, and `CONTRADICTION` once the set is `shipped`. It is not guessed into `approved` |
| [v1.11.0](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.11.0) | Five things a research set had to improvise by hand on its way to `shipped`, now in the registry and the audit. `--as-status <stage>` audits a set as if it were already at that stage, in memory and in place — the preview had been done on a copy outside the repo and came back with 54 findings, most of them anchors "missing" only because the copy was elsewhere; `--today` moves the date `accepted.until:` is measured against. `acceptance[]` gains `status: retired` (`retired_on:` + `retired_because:`): it does not block `shipped` and its citations keep resolving — three criteria had been deleted and their mentions rewritten by hand. Check 35, `evidence.derived_from:` — a conclusion read off measurements already in the registry inherits their `n:`/`spread:` instead of copying them, and is `measured` only while every source is; an asserted, missing, retracted or circular source is a `CONTRADICTION`. Check 36 makes `retracted_on`/`retracted_by` and `corrected_on`/`correction` first class: a citation leaning on a retracted entry without saying so is `DRIFT`, in this set and in any set citing it qualified; a citation of a corrected entry is a candidate, because on the real sets 7 of 9 cited a version already fixed in place. Accepted risks due within 14 days are listed apart and counted in the verdict. Also: `entorno:` in the template, for facts about an environment the feature runs inside and does not control, each with its own basis; gap-sweep layers `payments-onprem` and `web-3d-client`; and check 18 resolves anchors through route groups (`app/(tabs)/index.tsx`) and scoped packages (`node_modules/@d11/…`) without swallowing the paren of a parenthesised anchor. A registry that uses none of the new fields audits exactly as before |
| [v1.10.1](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.10.1) | v1.10.0 shipped `triage.py` with no test, and the omission was worst exactly there: the script's claim is that ranking is a layer and never a dependency — that with no SDK and no key the run still prints a complete list — and that path was the untested one. `tests/test_triage.py`, eight checks, none of which makes a call: `collect()` reproduces `audit.py`'s candidate list exactly, compared both ways against the report it prints, so a check silently narrowed and a candidate invented both fail; the fallback prints every candidate computed, because `suppressed` is a heading and not a filter; `--keep`/`--drop` are inclusive at the boundary and an unanswered candidate is `uncertain`, not suppressed; exits 0, 1 and 2 stay distinguishable for a caller; every generator reaches a question and check 8's two shapes get different ones; `line_at` returns nothing out of range rather than a neighbouring line. `Setup` is probed with `HOME` redirected, so the result does not depend on whether the machine running it happens to have a key |
| [v1.10.0](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.10.0) | `scripts/triage.py`, optional: v1.9.1 cut the four candidate generators down to what they should report, which fixed over-reporting but not reading order — a candidate list is still read top to bottom in the order `audit.py` emitted it, and the one that matters is as likely to sit last as first. The script asks one bounded question per candidate — is this the real thing, or one of the exempt shapes the protocol names in prose — and sorts them into `keep` / `uncertain` / `suppressed`. It writes no finding and assigns no severity: `audit.py` owns `## Findings`, and a probability is triage, never a verdict. Suppressed candidates are still printed, because a list nobody is told to read rebuilds the silent-skip failure `REQUIRES A HUMAN PASS` exists to close. It imports `audit.py` for the candidates rather than re-deriving them, so it can only reorder what the offline run already found — and with no `typesafe-sdk` and no API key it prints that same list unranked, which is a complete answer, not a degraded one: what is lost is reading order, not a check. `--no-model` takes that path deliberately, `--require-model` exits 2 rather than take it, `--check-setup` exits 1 reporting the gap |
| [v1.9.1](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.9.1) | v1.7.0's four candidate generators, measured against a corpus of real spec sets, over-reported on shapes the protocol already calls exempt — 114 candidates for four checks fell to 74, and one of the 114 pointed at a document that was correct. Check 3 conflated a contract entry's own descriptive keys (`note:`, `auth:`) with its fields — `{ fields: [...], note: "..." }` against a matching fence came back `missing {fields, note}` — and the strip now happens at the entry, never inside a payload, after a first by-name attempt turned 9 false positives into 13 different ones ([#15](https://github.com/CesarRivasP/feature-spec/issues/15)). Check 4 read a `§1–§5` legend as two dangling refs and a changelog row's enumeration as eleven; both now collapse ([#16](https://github.com/CesarRivasP/feature-spec/issues/16)). Check 8 read a pasted `package.json`/`tsconfig.json`/i18n bundle as an orphan contract — paste-ready doc 02 ships these by design — and now recognizes a named file, an unnamed manifest by its own keys, and a continuation fragment ([#17](https://github.com/CesarRivasP/feature-spec/issues/17)). Check 13's `dashboard` keyword was wrong 4 times out of 5 against an in-app screen; it now requires a named provider or collapses into a count ([#18](https://github.com/CesarRivasP/feature-spec/issues/18)) |
| [v1.9.0](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.9.0) | `changes[]` entries could rest on a `decisions.*` premise the same way a `defects[]` entry rests on another id, but a decision has no `status: dead` to cascade from — the convention is to edit `what:` in place, not mint a new key. An entry kept pointing at a directory a later decision had retired; nothing connected the two, so it rode through `implement` unchallenged. `decisions.*` gains optional `revised_on:`, `changes[]` gains optional `depends_on: [decisions.<key>]` + `reviewed_on:`. Check 34: `depends_on` names a decision revised more recently than the entry's own `reviewed_on:` → `DRIFT` |
| [v1.8.2](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.8.2) | Socket's audit of v1.8.1 flagged two real findings in `render.py`: a markdown link's URL reached the rendered `href` with no scheme check and quotes left unescaped (`[x](javascript:...)` survived verbatim), and `docs[].file` was joined onto `spec_dir` with no bound and its content *read* into the page — worse than `audit.py`'s check 20, which only checks existence. Every href/src now runs through a scheme allowlist, and `confined()` — the same guard v1.8.1 gave the anchor and profile checks — moved into `render.py` so `docs[].file` is bound before anything is read |
| [v1.8.1](https://github.com/CesarRivasP/feature-spec/releases/tag/v1.8.1) | three checks joined a registry path onto the repo root and opened whatever came back — `docs/../../outside/x:9` resolved exactly as written, so an anchor reported a stranger's line count and a `profile:` was read, parsed and had its `repo:` printed in a finding. A registry travels between repos and agents; every path in it is now confined to the checkout before it is opened, same rule that already keeps `evidence.cmd` from running. Checks 15, 18, 20 |
