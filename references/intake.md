...
- **Can the preconditions be met, on that run, today?** Every key in `_profile.yml conditions_required:` — the stream, the build type, the device class, the region — has to hold a known value *before* the procedure starts, not be read off afterwards. **If one cannot be met, the run does not happen.** A confound marked before the run and run anyway cost a retraction and a full device cycle; the result is `evidence.outcome: aborted_no_conditions` with `aborted_because:`, and it is never reinterpreted into a weak measurement. `references/evidence.md` §A run without its precondition produces no datum.
- **How many runs, and does the value vary between them?** `n:` is asked, not inferred. If the procedure is cheap enough to repeat, repeat it and record `spread:`; if it is not, `n: 1` is a legitimate answer and gets written as one. What is not legitimate is a single run entering the registry as a bare number — that is how it leaves the set as a constant.
- **If the observation is expected to be an ABSENCE, which line emits?** Before designing a procedure whose success condition is "nothing shows up", name the `file:line` that would have produced the signal and the sampling rate on it. A path that does not emit makes the whole run unable to answer the question. `references/gap-sweep.md` §Concluding from silence.
